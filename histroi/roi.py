...
In brief, the following functions are provided by the module:
    ...
    create_histo        |   extract a matching ROI from the histology image
            ...
            for layer in annotations.index.unique():
                ...
                if p.mask:
                    ...
                    mask = create_mask(
                        ...
                        scale_x=scale_x, dimscale_x=p.dimscale_x,
                        scale_y=scale_y, dimscale_y=p.dimscale_y, tile=p.tile,
                    ...
                    if p.histo:
                        histo = create_histo(
                            img=p.image, dimlevel=p.dimlevel,
                            original_shape=p.original_shape,
                            target_shape=p.target_shape, scale_x=scale_x,
                            dimscale_x=p.dimscale_x, scale_y=scale_y,
                            dimscale_y=p.dimscale_y, tile=p.tile)
...
def fix_polygon(*polygons):
    ...
    for i, p in enumerate(polygons):
        if not p.is_valid:
            p = p.buffer(0)
            if not p.is_valid:
                logger.critical("Fixing polygon {} was unsuccessful."
                                .format(i + 1))
            else:
                logger.info("Fixing polygon {} was successful.".format(i + 1))
        fixed.append(p)
...
def rasterise(geom, canvas, fill_value, xmin, ymin, scale_x, dimscale_x,
              scale_y, dimscale_y, mode):
    """
    ...
    :type scale_x: float
    :param dimscale_x:
        horizontal scaling factor pertaining to the resolution level
    :type dimscale_x: float
    :param scale_y: vertical scaling factor (output/original)
    :type scale_y: float
    :param dimscale_y:
        vertical scaling factor pertaining to the resolution level
    :type dimscale_y: float
    :param mode:
    ...
    def _rasterise_convex(ex, ey, canvas, fill_value, xmin, ymin,
                          scale_x, dimscale_x, scale_y, dimscale_y):
    ...
    if hasattr(geom, "geoms"):
        for geom in geom.geoms:
            canvas = rasterise(geom, canvas, fill_value, xmin, ymin,
                               scale_x, dimscale_x, scale_y, dimscale_y, mode)
    else:
        if mode == "exterior":
            ex, ey = geom.exterior.xy
            canvas = _rasterise_convex(ex, ey, canvas, fill_value, xmin, ymin,
                                       scale_x, dimscale_x, scale_y, dimscale_y)
        elif mode == "interior":
            for interior in geom.interiors:
                ex, ey = interior.xy
                canvas = _rasterise_convex(ex, ey, canvas, fill_value,
                                           xmin, ymin, scale_x, dimscale_x,
                                           scale_y, dimscale_y)
        else:
...
def get_fov(original_shape, target_shape, scale_x, scale_y, tile):
    """
    Determine the spatial extent of the output in pixels.

    :param original_shape:
        shape of the original histological image
        (in pixels, at the previously specified resolution level)
    :type original_shape: tuple[int, int]
    :param target_shape:
        Specifies the exact shape of the output in pixels. This functionality
        overrides the output shape definition by scaling factors to avoid
        rounding errors. Set this to None to calculate the output shape based
        on scaling factors.
    :type target_shape: Union[tuple[int, int], None]
    :param scale_x:
    ...
    :type scale_y: float
    :param tile:
        Tile specification. The specification should be in the following
        format: (x, y, width, height), where x and y refer to the horizontal
        and vertical coordinates of the top-left corner of the tile
        (w.r.t the output reference frame), and width, legth specify the shape
        of the tile in pixels.
    :type tile: Union[int, int, int, int]

    :return:
        Output of xmin, xmax, ymin, ymax, shape, scale_x, scale_y.
        The scale parameters represent scaling between the specified
        resolution level and the output shape. The reason why they are
        returned by this function is because exact target shape specification
        overrides the input scaling factors. The output represents factors that
        will be used to extract the histology image and to compute the new
        location of the annotation points before annotations are rasterised.
    :rtype: tuple[int, int, int, int tuple[int, int], float, float]

    """
    # Adjust scale parameters if the target shape is specified explicitly
    if target_shape is not None:
        scale_y, scale_x = np.divide(target_shape[::-1], original_shape)
    ...
    return xmin, xmax, ymin, ymax, shape, scale_x, scale_y


def create_mask(selection, original_shape=None, target_shape=None, scale_x=1,
                scale_y=1, dimscale_x=1, dimscale_y=1, invert=False, tile=None,
                fill_value=1):
    """
    ...
    :type scale_x: Number
    :param dimscale_x:
        Horizontal scale of the resolution level.
    :type dimscale_x: float
    :param dimscale_y:
        Vertical scale of the resolution level.
    :type dimscale_y: float
    :param scale_y:
    ...
    """
    # Define FOV
    xmin, xmax, ymin, ymax, shape, scale_x, scale_y = \
        get_fov(original_shape, target_shape, scale_x, scale_y, tile)
    logger.info("Binary mask FOV (pixels): {}".format(shape))

    # Define canvas
    canvas = np.zeros(shape, dtype=np.uint8, order="C")

    # Fill exterior contour and clear internal convex spaces
...
def create_histo(img, dimlevel, original_shape, target_shape, scale_x,
                 dimscale_x, scale_y, dimscale_y, tile):
    """
    Extract a ROI from the original histological image at the specified
    resolution.

    :param img: path to input image (*.svs)
    :type img: str
    :param dimlevel: reolution level (SVS standard, 0: highest, -1: lowest)
    :type dimlevel: int
    :param original_shape:
        If the original shape of the histology image (in pixels) is specified,
        the mask will be returned with a full field-of-view (FOV). The shape
    ...
    :return: histological image matched with the specified ROI
    :rtype: np.ndarray
    ...
    xmin, xmax, ymin, ymax, shape, scale_x, scale_y = \
        get_fov(original_shape, target_shape, scale_x, scale_y, tile)
    logger.info("Histology FOV (pixels): {}".format(shape))

    # Load image
    try:
        slideobj = openslide.open_slide(img)
        xmin0 = int(round(xmin / scale_x / dimscale_x))  # full-res
        xmin1 = int(round(xmin / scale_x))  # init-res
        ymin0 = int(round(ymin / scale_y / dimscale_y))  # full-res
        ymin1 = int(round(ymin / scale_y))  # init-res
        sy = min(int(round(shape[0] / scale_y)),
                 slideobj.level_dimensions[dimlevel][1] - ymin1)
        sx = min(int(round(shape[1] / scale_x)),
                 slideobj.level_dimensions[dimlevel][0] - xmin1)
        dimlevel = dimlevel % slideobj.level_count
        histo = slideobj.read_region((xmin0, ymin0), dimlevel, (sx, sy))
        histo = np.asarray(histo)
    except Exception as exc:
        raise exc

    # Rescale histology image
    if not np.allclose([scale_x, scale_y], 1.):
        histo = zoom(histo, (scale_y, scale_x, 1))
        # histo = rescale_intensity(histo, out_range=np.uint8)

    return np.asarray(histo, dtype=np.uint8)
